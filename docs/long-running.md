...
| Copied-out shared-memory function containers (`FunctionLikeTrait::copyOutOfSharedMemory()`) and the SHM originals they replace | opcache SHM is never written or freed; the writable `zend_function` container is malloc-backed (the engine's table destructor releases the body but never frees user function containers); bounded to one container per copied-out function |
| Added-method `zend_function` and added-constant `zend_class_constant` containers (`ClassDelta::apply()`) | published table buckets reference them until the class dies; the engine releases the bodies/values they carry but never frees the containers themselves (it assumes arena storage); malloc-backed, bounded to one per added member |
| Removed-method structures and their bodies (`ClassDelta::apply()`) | warmed-up inline caches and subclass method buckets may still point at the unpublished entry, so neither the structure nor its opcodes can be freed; bounded to one body per removed method |
| Previous function body of an in-flight `redefine()` (the entry was on the VM call stack during the swap) | the running frame still executes the old opcodes; the swap keeps the body allocated instead of freeing memory under the VM; bounded to one body per such self-redefinition |
| Final static-defaults duplicate of a redefined entry whose previous body outlives it (a live closure template holds the last body share) | the shared body arrays are freed by their last holder through *its* struct pointers, which do not know the entry's own defaults duplicate; bounded to one table per redefined entry (each swap releases the previous duplicate eagerly) |
...
| Arena-mimicking blocks of classes copied out of opcache shared memory (`ReflectionClass::copyOutOfSharedMemory()`) | the same copy model as a specialized class (row below): the copy is request memory reclaimed by the allocator at request end, while the shared-memory original it replaces in the class table is never written and never freed. Bounded to one copy per mutated shared-memory class per request |
